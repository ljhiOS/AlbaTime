...
struct ScheduleImportResultList: View {
    @ObservedObject var viewModel: ScheduleImportViewModel
    ...
    var body: some View {
        List {
            ...
            // 2. 스케줄 리스트 섹션 (인라인 편집)
            Section {
                if viewModel.parsedSchedules.isEmpty {
                    emptyStateView
                } else {
                    // 🔥 [핵심] $viewModel.parsedSchedules로 접근하여 Binding을 전달
                    ForEach($viewModel.parsedSchedules) { $schedule in
                ...
                HStack {
                    Text("인식된 스케줄 (\(viewModel.parsedSchedules.count)건)")
                    Spacer()
                    // 헤더에 [+] 버튼 배치
                    Button {
    ...
    private func deleteSchedule(at offsets: IndexSet) {
        viewModel.parsedSchedules.remove(atOffsets: offsets)
    }
...
#Preview {
    ScheduleImportResultList(viewModel: ScheduleImportViewModel())
}
