...
class ScheduleImportViewModel: ObservableObject {
    ...
    func saveToWorkplace(
        ...
        do {
            try scheduleSaving.execute(
                .editDraft(
                    jobID: session.editingJobID,
                    draft: session.scheduleImportDraft.makeEditDraft(
                        state: .existingJobAIImport,
                        targetWeekStart: targetWeekStart
                    )
                )
            )
    ...
    func saveSchedulePanelDraft(
        ...
        if let editingJobID = session.editingJobID {
            try scheduleSaving.execute(.editDraft(jobID: editingJobID, draft: draft))
            return
        }
