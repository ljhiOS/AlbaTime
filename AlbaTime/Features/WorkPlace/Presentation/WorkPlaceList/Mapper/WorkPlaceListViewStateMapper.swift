//
//  WorkPlaceListViewStateMapper.swift
//  AlbaTime
//
...
// TODO: WorkPlace 모델 정리 이후 WorkPlaceListViewState의 책임 범위 재검토

@MainActor
enum WorkPlaceListViewStateMapper {
    static func makeItem(from workPlace: WorkPlace) -> WorkPlaceListItemViewState {
        WorkPlaceListItemViewState(
            card: makeCard(from: workPlace),
            detail: makeDetail(from: workPlace),
            editingSeed: WorkPlaceEditingSeedFactory.make(from: workPlace)
        )
    }

    static func makeCard(from workPlace: WorkPlace) -> WorkPlaceCardViewState {
        WorkPlaceCardViewState(
            id: workPlace.id,
            name: workPlace.name,
...
private extension WorkPlaceListViewStateMapper {
